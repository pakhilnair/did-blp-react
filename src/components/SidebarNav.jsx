...
const SidebarNav = ({ toggleSidebar }) => {
  return (
    <div
      className="flex flex-col py-10 fixed top-0 left-0 h-full w-72 z-10 md:w-96 bg-gray-900 text-white transform 
                translate-x-0 transition-transform duration-300 ease-in-out overflow-y-auto"
    >
      <NavLink to="/">
        <img src="/logo.png" alt="Brainloop logo" className="max-w-32 ml-10" />
      </NavLink>
      <div className="border-t border-gray-700 mt-6 pb-1" />
      <div className="mt-6 mr-2">
        {sidebarItems.map((item, index) => (
          <div onClick={() => toggleSidebar()} key={index}>
            <NavLink
              to={item.route}
              className={({ isActive }) =>
